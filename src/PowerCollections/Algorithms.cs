...
namespace Wintellect.PowerCollections
{
    ...
    public static class Algorithms
    {
        ...
        /// <returns></returns>
        public static T[] RemoveDuplicates<T>(T[] inputArray,out  int counter)
        {
            counter = 0;
            HashSet<T> distinctArray = new HashSet<T>();
            for (int i = 0; i < inputArray.Length; i++)
            {
                if (distinctArray.Contains(inputArray[i]))
                {
                    counter++;
                    continue;
                }
                distinctArray.Add(inputArray[i]);
            }
            inputArray = new T[distinctArray.Count];
        ...
        /// <param name="y"></param>
        /// <returns></returns>
        public static int TailRecursionCalcGCD(int x, int y)
        {
            if (y == 0)
            {
                return x;
